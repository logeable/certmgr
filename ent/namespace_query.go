...
func (nq *NamespaceQuery) QueryCertificates() *CertificateQuery {
	query := (&CertificateClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, namespace.CertificatesTable, namespace.CertificatesColumn),
		)
...
func (nq *NamespaceQuery) Clone() *NamespaceQuery {
	...
	return &NamespaceQuery{
		...
		withCertificates: nq.withCertificates.Clone(),
...
func (nq *NamespaceQuery) WithCertificates(opts ...func(*CertificateQuery)) *NamespaceQuery {
	query := (&CertificateClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withCertificates = query
	return nq
}
...
func (nq *NamespaceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Namespace, error) {
	...
	if query := nq.withCertificates; query != nil {
		if err := nq.loadCertificates(ctx, query, nodes,
			func(n *Namespace) { n.Edges.Certificates = []*Certificate{} },
			func(n *Namespace, e *Certificate) { n.Edges.Certificates = append(n.Edges.Certificates, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
